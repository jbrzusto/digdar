...
int rp_osc_get_signals(float ***signals, int *sig_idx)
{
    ...
    memcpy(&s[4][0], &rp_osc_signals[4][0], sizeof(float)*SIGNAL_LENGTH);
...
int rp_osc_set_signals(float **source, int index)
{
    ...
    memcpy(&rp_osc_signals[3][0], &source[3][0], sizeof(float)*SIGNAL_LENGTH);
    memcpy(&rp_osc_signals[4][0], &source[4][0], sizeof(float)*SIGNAL_LENGTH);
    ...
    while(1) {
        ...
        if((state != old_state) || params_dirty)
            ...
            rp_osc_decimate((float **)&rp_tmp_signals[1], &rp_fpga_cha_signal[0],
                            ...
                            (float **)&rp_tmp_signals[3], &rp_fpga_xcha_signal[0],
                            (float **)&rp_tmp_signals[4], &rp_fpga_xchb_signal[0],
 ...
 * @param [out] xcha_signal - processed output signal for Slow Channel A
 * @param [in] in_xcha_signal - input raw signal for Slow Channel A
 * @param [out] xchb_signal - processed output signal for Slow Channel B
 * @param [in] in_xchb_signal - input raw signal for Slow Channel B
...
int rp_osc_decimate(float **cha_signal, int *in_cha_signal,
                    ...
                    float **xcha_signal, int *in_xcha_signal,
                    float **xchb_signal, int *in_xchb_signal,
                    ...
                    float t_start, float t_stop, int time_unit)
{
    ...
    float *xcha_s = *xcha_signal;
    float *xchb_s = *xchb_signal;
    float *t = *time_signal;
    ...
    for(out_idx=0, t_idx=0; out_idx < SIGNAL_LENGTH; 
        ...
        xcha_s[out_idx] = osc_fpga_cnv_xcnt_to_v(in_xcha_signal[in_idx]);
        xchb_s[out_idx] = osc_fpga_cnv_xcnt_to_v(in_xchb_signal[in_idx]);
 ...
 * @param [out] xcha_out_signal - processed output signal for Slow Channel A
 * @param [in] xcha_in_signal - input raw signal for Slow Channel A
 * @param [out] xchb_out_signal - processed output signal for Slow Channel B
 * @param [in] xchb_in_signal - input raw signal for Slow Channel B
...
int rp_osc_decimate_partial(float **cha_out_signal, int *cha_in_signal, 
                            ...
                            float **xcha_out_signal, int *xcha_in_signal,
                            float **xchb_out_signal, int *xchb_in_signal,
                            ...
                            float t_start, int dec_factor, int time_unit)
{
    ...
    float *xcha_out = *xcha_out_signal;
    float *xchb_out = *xchb_out_signal;
    ...
    for(; (next_out_idx < SIGNAL_LENGTH); next_out_idx++, 
        ...
        xcha_out[next_out_idx] = osc_fpga_cnv_cnt_to_v(xcha_in_signal[in_idx]);
        xchb_out[next_out_idx] = osc_fpga_cnv_cnt_to_v(xchb_in_signal[in_idx]);
        t_out[next_out_idx]   = 
