...
/* @brief Pointer to data buffer where signal on slow channel A is captured.  */
static uint32_t           *g_osc_fpga_xcha_mem = NULL;
...
/* @brief Number of slow ADC acquisition bits.  */
...
/*----------------------------------------------------------------------------*/
/**
 * @brief Retrieve the address of channel A,B and slow channel A,B memory buffers
 *
 ...
 * @param[out] xcha_signal Pointer to slow channel A memory buffer
 * @param[out] xchb_signal Pointer to slow channel B memory buffer
 * @retval 0 Success, never fails
 */
