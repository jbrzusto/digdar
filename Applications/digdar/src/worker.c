    ...
    while(1) {
        ...
        if(!long_acq) {
            ...
            long_acq_idx = rp_osc_decimate_partial((float **)&rp_tmp_signals[1], 
                                             ...
                                             (float **)&rp_tmp_signals[3],
                                             &rp_fpga_xcha_signal[0],
                                             (float **)&rp_tmp_signals[4],
                                             &rp_fpga_xchb_signal[0],
                    ...
                    float ch1_user_dc_off, float ch2_user_dc_off)
{
    ...
    for(out_idx=0, t_idx=0; out_idx < SIGNAL_LENGTH; 
        ...
        cha_s[out_idx] = osc_fpga_cnv_cnt_to_rel(in_cha_signal[in_idx], c_osc_fpga_adc_bits);

        chb_s[out_idx] = osc_fpga_cnv_cnt_to_rel(in_chb_signal[in_idx], c_osc_fpga_adc_bits);

        xcha_s[out_idx] = osc_fpga_cnv_cnt_to_rel(in_xcha_signal[in_idx], c_osc_fpga_xadc_bits);
        xchb_s[out_idx] = osc_fpga_cnv_cnt_to_rel(in_xchb_signal[in_idx], c_osc_fpga_xadc_bits);
                            ...
                            float ch1_user_dc_off, float ch2_user_dc_off)
{
    ...
    for(; (next_out_idx < SIGNAL_LENGTH); next_out_idx++, 
        ...
        xcha_out[next_out_idx] = osc_fpga_cnv_xcnt_to_v(xcha_in_signal[in_idx]);
        xchb_out[next_out_idx] = osc_fpga_cnv_xcnt_to_v(xchb_in_signal[in_idx]);
