...
int spectr_fpga_update_params(/* float trig_excite, float trig_relax, int trig_delay, int trig_latency,*/
                              int freq_range, int enable_avg_at_dec)
{
    ...
    /* convert trigger excite / relax levels to raw signed 14-bit units */
    /* g_digdar_fpga_reg_mem->trig_excite = 8192 + 8192.0 * trig_excite; */
    /* g_digdar_fpga_reg_mem->trig_relax = 8192 + 8192.0 * trig_relax; */

    /* g_digdar_fpga_reg_mem->trig_delay = trig_delay; */
...
int spectr_fpga_set_trigger()
{
    g_spectr_fpga_reg_mem->trig_source = 10; // digdar counted trigger pulse
    return 0;
}
